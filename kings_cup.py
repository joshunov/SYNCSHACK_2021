...
def KingsCup(playerlist):
    ...
    while True:
        ...
        input("It's {}'s turn. Press ENTER to continue.".format(currentPlayer))

        card = random.choice(list(cardCount.keys()))
        print("{} has pulled the card \"{}\"".format(currentPlayer, card))
        cardCount, gameOver = ActionCardOutcome(currentPlayer, card, cardCount)

        if gameOver < 0:
            break

        input("Press ENTER to continue, once the task is finished!")
    ...
    print("You have finished King's cup - returning you to the main menu!")
...
def ActionCardOutcome(player, cardOutcome, remainingDeck):
    ...
    if cardOutcome == "Ace":
        ...
        print("Categories!")
        print("{} picks a category, others need to say things in that category. First to not be able to drinks.")
    elif cardOutcome == "9":
        print("Bust a Rhyme!")
        print("Pick a word, others must rhyme it. First to not be able to drinks.")
    elif cardOutcome == "8":
        print("Mate!")
        print("{} - find a drinking buddy. You both drink together for the remainder of the game.")
    elif cardOutcome == "7":
        print("Heaven!")
        ...
        print("Bust a jive!")
        print("Create a dance move; the one who srews it up drinks.")
    elif cardOutcome == "4":
        print("Whores!")
        print("Ladies drink.")
    elif cardOutcome == "3":
        ...
        print("You!")
        print("{} gets to pick someone to take two drinks.")
...
if __name__ == "__main__":
    KingsCup(["Jordan", "Josh", "Adam", "Cam", "Rishi", "Hamish"])